...
(function($) {
    ...
    $.fn.fzValidate = function(options) {
        return this.each(function() {
            ...
            var vObj;
            var errboxid = $this.attr('id') + 'ErrMsg';
            //渲染
            function render() {
                if (opts.type === 'modal') {
                    ...
                    $this.append('<div id=' + errboxid + ' style="display:none;"><div>');
                    vObj = $this.validate($.extend({
                        focusInvalid: false,
                        focusCleanup: true,
                        showErrors: function(errorMap, errorList) {
    ...
    $.fn.fzTable = function(options) {
        return this.each(function() {
            ...
            function init() {
                ...
                that.getChooseRows = function() {
                    var rows = $this.bootstrapTable('getSelections');
                    return (rows.length == 1 && opts.singleSelect) ? rows[0] : rows;
                };
                //获取选中行的数据中指定KEY数组
                that.getChooseRowsKeys = function(key) {
                    return $.map(that.getChooseRows(), function(n) {
                        return n[key];
                    });
                };
                //获取选中行的数据ID数组
    ...
    $.fn.fzTreeTable = function(options) {
        ...
        function addTr(jqDom, node, deep) {
            ...
            ttDatas[id] = { id: id, parentId: pid, deep: deep };
